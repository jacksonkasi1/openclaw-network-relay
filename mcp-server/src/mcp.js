...
const NOISE_DOMAINS = [
  ...
  'amplitude.com', 'fullstory.com', 'intercom.io', 'intercom.com',
  'datadoghq.com', 'newrelic.com', 'nr-data.net', 'rollbar.com',
  'sentry.io', 'bugsnag.com', 'logrocket.com', 'mouseflow.com',
  'heapanalytics.com', 'crazyegg.com', 'optimizely.com', 'launchdarkly.com',
  'analytics.twitter.com', 'ads.linkedin.com', 'snap.licdn.com',
  'bat.bing.com', 'connect.facebook.net', 'cdn.branch.io',
  'cdn.mxpnl.com', 'cdn.segment.com', 'cdn.amplitude.com'
];
...
const DEVICE_PRESETS = {
  'iPhone 15':        { width: 393,  height: 852,  deviceScaleFactor: 3,     mobile: true,  ua: 'Mozilla/5.0 (iPhone; CPU iPhone OS 17_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/17.0 Mobile/15E148 Safari/604.1' },
  ...
  'Pixel 8':          { width: 412,  height: 915,  deviceScaleFactor: 2.625, mobile: true,  ua: 'Mozilla/5.0 (Linux; Android 14; Pixel 8) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Mobile Safari/537.36' },
  'Samsung Galaxy S24':{ width: 360, height: 780,  deviceScaleFactor: 3,     mobile: true,  ua: 'Mozilla/5.0 (Linux; Android 14; SM-S921B) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Mobile Safari/537.36' },
  ...
  'reset':            { width: 1280, height: 800,  deviceScaleFactor: 1,     mobile: false, ua: '' },
};

const FUZZ_PAYLOADS = {
  sqli_basic:         ["'", '"', "' OR '1'='1", "' OR 1=1--", "'; DROP TABLE users--", "1 UNION SELECT null--", "admin'--", "' AND SLEEP(5)--", "1; WAITFOR DELAY '0:0:5'--", "' HAVING 1=1--", "' OR 'x'='x", "\" OR \"x\"=\"x"],
  xss_basic:          ['<script>alert(1)</script>', '"><script>alert(1)</script>', "'><script>alert(1)</script>", "';alert(1)//", '<img src=x onerror=alert(1)>', '<svg onload=alert(1)>', '<iframe src=javascript:alert(1)>', '{{7*7}}', '${7*7}', '<body onload=alert(1)>'],
  path_traversal:     ['../etc/passwd', '../../etc/passwd', '../../../etc/passwd', '..%2F..%2Fetc%2Fpasswd', '....//....//etc/passwd', '%2e%2e%2f%2e%2e%2f', '..%5c..%5c', '/etc/passwd', 'C:\\Windows\\System32\\drivers\\etc\\hosts', '....\\\\....\\\\etc\\\\passwd'],
  nosqli_basic:       ['{"$gt": ""}', '{"$ne": null}', '{"$regex": ".*"}', "' || '1'=='1", '{"$where": "1==1"}', '{$gt: ""}', '[$ne]=1', '{"$gt":""}'],
  template_injection: ['{{7*7}}', '${7*7}', '<%= 7*7 %>', '#{7*7}', '*{7*7}', '{{config}}', '{{self.__dict__}}', '${T(java.lang.Runtime).getRuntime().exec("id")}'],
};
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    if (request.params.name === "get_traffic_history") {
      ...
      if (args.log_id) {
        ...
        if (args.skip_noise) {
          history = history.filter(h => !NOISE_DOMAINS.some(domain => h.url && h.url.includes(domain)));
        }
        if (args.light_mode) {
    ...
    if (request.params.name === "get_traffic_detail") {
      const args = request.params.arguments || {};
      const { log_id } = args;
      if (!log_id) return { isError: true, content: [{ type: "text", text: "log_id is required." }] };
      const rows = executeRawQuery(`SELECT * FROM traffic_logs WHERE id = '${log_id.replace(/'/g, "''")}'`);
      if (!rows || rows.length === 0) return { content: [{ type: "text", text: `No traffic log found with id: ${log_id}` }] };
    ...
    if (request.params.name === "browser_wait_for") {
      ...
      try {
        ...
        if (args.wait_type === 'selector') {
          if (!args.selector) return { isError: true, content: [{ type: "text", text: "selector is required when wait_type=selector" }] };
        ...
        const res = await sendCdpCommand(null, "Runtime.evaluate", { expression, returnByValue: true, awaitPromise: true });
        if (res.exceptionDetails) return { isError: true, content: [{ type: "text", text: "Wait failed: " + res.exceptionDetails.exception.description }] };
        return { content: [{ type: "text", text: "Wait complete: " + JSON.stringify(res.result.value) }] };
      } catch (e) {
        return { isError: true, content: [{ type: "text", text: e.message }] };
      }
    }
    ...
    if (request.params.name === "browser_handle_dialog") {
      ...
      try {
        ...
        return { content: [{ type: "text", text: `Dialog ${args.action}ed successfully.` }] };
      } catch (e) {
        return { isError: true, content: [{ type: "text", text: e.message }] };
      }
    }
    ...
    if (request.params.name === "browser_get_console_logs") {
      ...
      try {
        // Install hook if not present (idempotent)
        const hookExpr = `(() => {
          ...
          ['log','warn','error','info','debug'].forEach(level => {
            ...
            console[level] = (...args) => {
              window.__openclawConsoleLogs.push({ level, msg: args.map(a => { try { return typeof a === 'object' ? JSON.stringify(a) : String(a); } catch(e) { return '[unserializable]'; } }).join(' '), t: Date.now() });
              _orig[level](...args);
            };
          });
          return 'hooked';
        })()`;
        await sendCdpCommand(null, "Runtime.evaluate", { expression: hookExpr, returnByValue: true });

        // Retrieve logs
        const getExpr = `(() => {
          const logs = window.__openclawConsoleLogs || [];
          const filtered = ${args.level_filter && args.level_filter.length ? `logs.filter(l => ${JSON.stringify(args.level_filter)}.includes(l.level))` : 'logs'};
          const result = filtered.slice(-${limit});
          ${args.clear ? 'window.__openclawConsoleLogs = [];' : ''}
          return result;
        })()`;
        const res = await sendCdpCommand(null, "Runtime.evaluate", { expression: getExpr, returnByValue: true });
        if (res.exceptionDetails) return { isError: true, content: [{ type: "text", text: "Exception: " + res.exceptionDetails.exception.description }] };
        const logs = res.result.value || [];
        if (logs.length === 0) return { content: [{ type: "text", text: "No console messages (hook installed — future messages will be captured)." }] };
    ...
    if (request.params.name === "browser_hover") {
      ...
      try {
        ...
        const coordRes = await sendCdpCommand(null, "Runtime.evaluate", { expression: coordExpr, returnByValue: true });
        if (!coordRes.result?.value) return { isError: true, content: [{ type: "text", text: "Element not found for hover. Did you run browser_extract_dom first?" }] };
        const { x, y } = coordRes.result.value;
        await sendCdpCommand(null, "Input.dispatchMouseEvent", { type: "mouseMovedToElement", x, y, modifiers: 0 });
        await sendCdpCommand(null, "Input.dispatchMouseEvent", { type: "mouseMoved", x, y, modifiers: 0 });
        ...
        return { content: [{ type: "text", text: `Hovered over element at (${x}, ${y}).` }] };
      } catch (e) {
        return { isError: true, content: [{ type: "text", text: e.message }] };
      }
    }
    ...
    if (request.params.name === "browser_press_key") {
      ...
      if (!keyInfo) return { isError: true, content: [{ type: "text", text: `Unknown key: ${args.key}` }] };
      try {
        const base = { key: args.key, code: keyInfo.code, windowsVirtualKeyCode: keyInfo.windowsVirtualKeyCode, nativeVirtualKeyCode: keyInfo.windowsVirtualKeyCode, modifiers: 0 };
        if (keyInfo.text) base.text = keyInfo.text;
        await sendCdpCommand(null, "Input.dispatchKeyEvent", { ...base, type: "rawKeyDown" });
        if (keyInfo.text) await sendCdpCommand(null, "Input.dispatchKeyEvent", { ...base, type: "char" });
        await sendCdpCommand(null, "Input.dispatchKeyEvent", { ...base, type: "keyUp" });
    ...
    if (request.params.name === "browser_drag") {
      ...
      try {
        const getCoords = async (id, selector) => {
          ...
          const res = await sendCdpCommand(null, "Runtime.evaluate", { expression: expr, returnByValue: true });
          return res.result?.value || null;
        };

        const from = await getCoords(args.from_id, args.from_selector);
        const to = await getCoords(args.to_id, args.to_selector);
        if (!from) return { isError: true, content: [{ type: "text", text: "Source element not found." }] };
        if (!to) return { isError: true, content: [{ type: "text", text: "Target element not found." }] };

        const STEPS = 12;
        await sendCdpCommand(null, "Input.dispatchMouseEvent", { type: "mousePressed", x: from.x, y: from.y, button: "left", buttons: 1, clickCount: 1, modifiers: 0 });
        for (let i = 1; i <= STEPS; i++) {
          const x = Math.round(from.x + (to.x - from.x) * (i / STEPS));
          const y = Math.round(from.y + (to.y - from.y) * (i / STEPS));
          await sendCdpCommand(null, "Input.dispatchMouseEvent", { type: "mouseMoved", x, y, button: "left", buttons: 1, modifiers: 0 });
        }
        await sendCdpCommand(null, "Input.dispatchMouseEvent", { type: "mouseReleased", x: to.x, y: to.y, button: "left", buttons: 0, clickCount: 1, modifiers: 0 });
    ...
    if (request.params.name === "browser_emulate_device") {
      ...
      try {
        ...
        if (!cfg) cfg = { width: args.custom_width || 1280, height: args.custom_height || 800, deviceScaleFactor: 1, mobile: args.mobile || false, ua: args.custom_ua || '' };
        const ua = args.custom_ua || cfg.ua;

        await sendCdpCommand(null, "Emulation.setDeviceMetricsOverride", {
          width: cfg.width, height: cfg.height, deviceScaleFactor: cfg.deviceScaleFactor,
          mobile: cfg.mobile, screenWidth: cfg.width, screenHeight: cfg.height,
          screenOrientation: { type: cfg.height > cfg.width ? 'portraitPrimary' : 'landscapePrimary', angle: cfg.height > cfg.width ? 0 : 90 }
        });
        if (ua) await sendCdpCommand(null, "Emulation.setUserAgentOverride", { userAgent: ua });
        await sendCdpCommand(null, "Emulation.setTouchEmulationEnabled", { enabled: cfg.mobile, maxTouchPoints: cfg.mobile ? 5 : 0 });

        const label = args.preset || `custom ${cfg.width}x${cfg.height}`;
        return { content: [{ type: "text", text: `Device emulation set to: ${label}. Viewport: ${cfg.width}x${cfg.height}, Mobile: ${cfg.mobile}. Reload the page for full effect.` }] };
      } catch (e) {
        return { isError: true, content: [{ type: "text", text: e.message }] };
      }
    }
    ...
    if (request.params.name === "browser_memory_scan") {
      try {
        ...
        if (findings.length === 0) return { content: [{ type: "text", text: "No secrets found in JS-accessible memory. (Encrypted apps like 1Password store keys only in heap RAM — use browser_execute_cdp with HeapProfiler for deeper analysis.)" }] };
    ...
    if (request.params.name === "browser_websocket_hook") {
      try {
        ...
        const res = await sendCdpCommand(null, "Runtime.evaluate", { expression: hookExpression, returnByValue: true });
        if (res.exceptionDetails) return { isError: true, content: [{ type: "text", text: "Exception: " + res.exceptionDetails.exception.description }] };
        return { content: [{ type: "text", text: "WebSocket hook: " + JSON.stringify(res.result.value) + ". All future WebSocket connections will be captured." }] };
      } catch (e) {
        return { isError: true, content: [{ type: "text", text: e.message }] };
      }
    }
    ...
    if (request.params.name === "browser_websocket_send") {
      ...
      try {
        ...
        if (val?.error) return { isError: true, content: [{ type: "text", text: val.error }] };
        return { content: [{ type: "text", text: "Frame sent: " + JSON.stringify(val) }] };
      } catch (e) {
        return { isError: true, content: [{ type: "text", text: e.message }] };
      }
    }
    ...
    if (request.params.name === "browser_dom_diff") {
      ...
      try {
        ...
        if (args.reset || !lastDomSnapshot) {
          ...
          return { content: [{ type: "text", text: args.reset ? "DOM baseline reset." : "No previous snapshot — baseline stored. Make your action, then call browser_dom_diff again." }] };
        }

        // Compute line-level diff
        const oldLines = new Set(lastDomSnapshot.split('\n').map(l => l.trim()).filter(Boolean));
        const newLines = new Set(currentDom.split('\n').map(l => l.trim()).filter(Boolean));
        const added = [...newLines].filter(l => !oldLines.has(l)).map(l => '+ ' + l);
        const removed = [...oldLines].filter(l => !newLines.has(l)).map(l => '- ' + l);
        lastDomSnapshot = currentDom;

        if (added.length === 0 && removed.length === 0) return { content: [{ type: "text", text: "No DOM changes detected since last snapshot." }] };
        const diff = [...removed.slice(0, 100), ...added.slice(0, 100)].join('\n');
        return { content: [{ type: "text", text: `DOM diff (${removed.length} removed, ${added.length} added):\n${diff}` }] };
      } catch (e) {
        return { isError: true, content: [{ type: "text", text: e.message }] };
      }
    }
    ...
    if (request.params.name === "browser_fuzz") {
      const args = request.params.arguments || {};
      const { url, method, headers = {}, body_template, param_name, payload_type, num_start = 1, num_end = 100, custom_payloads = [], concurrency = 5, timeout_ms = 8000 } = args;
      ...
      try {
        ...
        if (payloads.length === 0) return { isError: true, content: [{ type: "text", text: "No payloads to fuzz with." }] };

        // Baseline request
        const buildRequest = (payload) => {
          let reqUrl = url.includes('{{FUZZ}}') ? url.replace(/\{\{FUZZ\}\}/g, encodeURIComponent(payload)) : url;
          if (param_name && !url.includes('{{FUZZ}}')) {
            const u = new URL(reqUrl);
            u.searchParams.set(param_name, payload);
            reqUrl = u.toString();
          }
          let reqBody = body_template ? body_template.replace(/\{\{FUZZ\}\}/g, payload) : undefined;
          const opts = { method: method.toUpperCase(), headers: { ...headers }, signal: AbortSignal.timeout(timeout_ms) };
          for (const k of ['host','content-length','connection']) delete opts.headers[k];
          if (reqBody && !['GET','HEAD'].includes(opts.method)) opts.body = reqBody;
          return { url: reqUrl, opts };
        };

        const baseline = buildRequest('BASELINE_OPENCLAW_TEST');
        let baselineStatus = 200, baselineLength = 0;
        try {
        ...
        const results = { total: payloads.length, baseline: { status: baselineStatus, length: baselineLength }, anomalies: [] };
          ...
          await Promise.all(batch.map(async (payload) => {
            try {
              ...
              if (statusChanged || lenDiff > 50) {
                results.anomalies.push({ payload: payload.substring(0, 80), status: r.status, length: body.length, lenDiff, preview: body.substring(0, 200) });
              }
            } catch(e) {
        ...
        if (results.anomalies.length === 0) return { content: [{ type: "text", text: `Fuzzing complete. ${results.total} payloads tested. No anomalies detected (all matched baseline status=${baselineStatus}, length≈${baselineLength}).` }] };
        return { content: [{ type: "text", text: JSON.stringify(results, null, 2) }] };
      } catch (e) {
        return { isError: true, content: [{ type: "text", text: "Fuzz error: " + e.message }] };
      }
    }

    if (request.params.name === "browser_pause_for_human") {
      const args = request.params.arguments || {};
      const message = args.message || "Please complete the verification and click Resume AI";
      ...
      try {
        ...
        await sendCdpCommand(null, "Runtime.evaluate", { expression: injectExpr, returnByValue: true });
        ...
        while (Date.now() - startMs < maxMs) {
          ...
          try {
            const checkRes = await sendCdpCommand(null, "Runtime.evaluate", { expression: "window.__openclawPaused === false ? 'resumed' : 'waiting'", returnByValue: true });
            if (checkRes.result?.value === 'resumed') {
              return { content: [{ type: "text", text: "Human resumed. Continuing..." }] };
            }
          } catch(e) { /* extension might briefly disconnect — keep polling */ }
        }
        // Timed out — clean up overlay
        await sendCdpCommand(null, "Runtime.evaluate", { expression: "document.getElementById('__openclaw_pause__')?.remove(); window.__openclawPaused = false;", returnByValue: true }).catch(() => {});
        return { content: [{ type: "text", text: `Human pause timed out after ${timeoutSeconds}s. Continuing anyway.` }] };
      } catch (e) {
        return { isError: true, content: [{ type: "text", text: e.message }] };
      }
    }
